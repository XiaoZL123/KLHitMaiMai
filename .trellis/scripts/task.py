...
Usage:
    py -3 task.py create "<title>" --description "<desc>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3] [--parent <dir>] [--package <pkg>] [--no-start] [--force]
    ...
    py -3 task.py start <dir>                 # Set active task, record current branch
    ...
    py -3 task.py set-meta <dir> <key> <value>  # Set a task metadata key
    py -3 task.py rename <dir> <new-slug> [--dry-run]  # Rename task + references
    py -3 task.py archive <task-dir> [--skip-branch-validation]  # Archive completed task
...
def _record_start_state(
    ...
    if base_branch_conflict:
        ...
        print(
            f"Once you branch off, run: py -3 {DIR_WORKFLOW}/scripts/task.py "
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "allow_empty_context", False):
        ...
        if empty_manifests:
            ...
            print(f"  Curate:  py -3 .trellis/scripts/task.py add-context {task_input} implement <path> \"<why>\"")
            print(f"  Verify:  py -3 .trellis/scripts/task.py validate {task_input}")
...
Usage:
  py -3 task.py create <title> --description <desc>  Create new task directory (both required, non-empty)
  py -3 task.py create <title> --description <desc> --package <pkg>   Create task for a specific package
  py -3 task.py create <title> --description <desc> --parent <dir>    Create task as child of parent
  py -3 task.py create <title> --description <desc> --no-start        Create without making it active in this session
  py -3 task.py add-context <dir> <jsonl> <path> [reason]  Add entry to jsonl
  py -3 task.py validate <dir>                     Validate jsonl files
  py -3 task.py list-context <dir>                 List jsonl entries
  py -3 task.py start <dir>                        Set active task; records the checked-out branch when unset
  py -3 task.py current [--source]                 Show active task
  py -3 task.py finish                             Clear active task
  py -3 task.py set-branch <dir> <branch>          Set git branch
  py -3 task.py set-base-branch <dir> <branch>     Set PR target branch
  py -3 task.py set-scope <dir> <scope>            Set scope for PR title
  py -3 task.py set-meta <dir> <key> <value>       Set/overwrite a task metadata key
  py -3 task.py rename <dir> <new-slug>            Rename task, identity fields and references
...
Examples:
  py -3 task.py create "Add login feature" --description "Email + password sign-in" --slug add-login
  py -3 task.py create "Add login feature" --description "Email + password sign-in" --slug add-login --package cli
  py -3 task.py create "Add login feature" --description "Email + password sign-in" --meta linear=ENG-123 --meta epic=auth
  py -3 task.py create "Child task" --description "Session cookie handling" --slug child --parent .trellis/tasks/01-21-parent
  ...
  py -3 task.py finish
  py -3 task.py rename add-login add-sso --dry-run  # Preview the change set
  py -3 task.py rename add-login add-sso
  py -3 task.py archive add-login
  py -3 task.py archive add-login --skip-branch-validation  # Task never had a branch of its own
