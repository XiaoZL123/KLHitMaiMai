    ...
    if broken:
        print(
            colored(
                ...
                "Re-link each one with `py -3 .trellis/scripts/task.py "
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    if created_jsonl:
        ...
        print(
            "      list available specs: py -3 .trellis/scripts/get_context.py --mode packages",
            file=sys.stderr,
        )
...
def _validate_branch_metadata(
    ...
    task_py = f"py -3 {DIR_WORKFLOW}/scripts/task.py"
...
def cmd_set_meta(args: argparse.Namespace) -> int:
    ...
    if not key:
        print(colored("Error: Missing arguments", Colors.RED))
        print("Usage: py -3 task.py set-meta <task-dir> <key> <value>")
        return 1
